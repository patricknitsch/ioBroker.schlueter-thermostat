...
class OJClient {
	/**
	 * @param {{log:any, baseUrlOwd5?:string, baseUrlOcd5?:string, username:string, password:string, apiKey:string, customerId:number, clientSwVersion:number}} cfg
	 */
	constructor(cfg) {
		this.log = cfg.log;
		...
		this.clientSwVersion = Number(cfg.clientSwVersion) || 1;
	...
	async login() {
		...
		const payload = {
			...
			ClientSWVersion: this.clientSwVersion,
		};

		this.log.debug(
			`OWD5 login: POST ${url} (CustomerId=${this.customerId}, ClientSWVersion=${this.clientSwVersion})`,
		);
	...
	async getEnergyUsage(thermostatId, opts) {
		await this.ensureSession();
		...
		this.log.debug(
			`GetEnergyUsage: POST ${url} (ThermostatID=${thermostatId}, ViewType=${payload.ViewType}, History=${payload.History}, DateTime=${payload.DateTime})`,
		);
		const { data } = await this.http.post(url, payload, { params: { sessionid: this.sessionId } });
		if (data?.ErrorCode && data.ErrorCode !== 0) {
