...
function createApplyRouter(adapter) {
	...
	const handlers = {
		comfort: async ({ devPrefix, serial, baseName, thermostatId }) => {
			...
			const tzSec = adapter.thermostatTimeZoneSec[thermostatId] ?? 0;
			const comfortEnd = nowPlusMinutesThermostatLocalNoZ(dur, tzSec);
		...
		boost: async ({ devPrefix, serial, baseName, thermostatId }) => {
			...
			const tzSec = adapter.thermostatTimeZoneSec[thermostatId] ?? 0;
			const boostEnd = nowPlusMinutesThermostatLocalNoZ(dur, tzSec);
