...
const I18N_LANGUAGES = ['en', 'de', 'ru', 'pt', 'nl', 'fr', 'it', 'es', 'pl', 'uk', 'zh-cn'];
const I18N_TEXT_KEYS = new Set(['identifierLabel', 'label', 'labelOn', 'labelOff', 'trueText', 'falseText', 'name']);
...
class SchlueterDeviceManagement extends DeviceManagement {
					...
					status: [
						...
						},
						{
							warning: {
								...
								mapping: {
									false: this._t('Heating off', 'Heizen aus'),
									true: this._t('Heating on', 'Heizen an'),
								},
							},
						},
						{
							warning: {
								...
								mapping: {
									1: this._t('Mode: Schedule', 'Modus: Zeitplan'),
									2: this._t('Mode: Comfort', 'Modus: Komfort'),
									3: this._t('Mode: Manual', 'Modus: Manuell'),
									4: this._t('Mode: Vacation', 'Modus: Urlaub'),
									6: this._t('Mode: Frost Protection', 'Modus: Frostschutz'),
									8: this._t('Mode: Boost', 'Modus: Boost'),
									9: this._t('Mode: Eco', 'Modus: Eco'),
								},
							},
						},
					],
	...
	getDeviceDetails(deviceId) {
		...
		return {
			id: deviceId,
			schema: this._expandI18n(this._detailsSchema(prefix, id, ids.groupId, ids.thermostatId)),
		};
	}
	...
	/** Expands EN/DE text to all adapter-supported i18n languages and returns a translated label object. */
	_t(en, de) {
		return this._expandI18n({ label: { en, de } }).label;
	}
	...
	/** Adds all adapter i18n languages to en/de text objects. */
	_expandI18n(value, parentKey = '') {
		if (Array.isArray(value)) {
			return value.map(v => this._expandI18n(v, parentKey));
		}
		...
		const isEnDeText =
			I18N_TEXT_KEYS.has(parentKey) &&
			keys.length > 0 &&
			keys.length <= 2 &&
			keys.every(k => k === 'en' || k === 'de') &&
			typeof value.en === 'string' &&
			(value.de === undefined || typeof value.de === 'string');
		if (isEnDeText) {
			const en = value.en;
			const de = value.de ?? en;
			const all = {};
			for (const lang of I18N_LANGUAGES) {
				all[lang] = lang === 'de' ? de : en;
			}
			return all;
		}

		for (const key of keys) {
			value[key] = this._expandI18n(value[key], key);
		}
		return value;
	}
