...
class SchlueterDeviceManagement extends DeviceManagement {
	...
	async loadDevices(context) {
		...
		for (const { device, ids } of addableDevices) {
			...
			const onlineState = await this.adapter.getStateAsync(
				`${this.adapter.namespace}.${this._prefix(groupId, thermostatId)}.online`,
			);
			...
			context.addDevice({
				id: dmDeviceId,
				name: thermostatName,
				icon: `/adapter/${this.adapter.name}/schlueter-thermostat.jpg`,
				status: {
					connection: {
				...
				color: !isOnline ? '#fff' : undefined,
				backgroundColor: !isOnline ? '#f44336' : undefined,
				controls: [],
				actions: [],
				hasDetails: true,
				customInfo: {
	...
	/** @returns {Record<string, import('@iobroker/dm-utils').ConfigItemPanel['items'][string]>} shared measurement field definitions */
	_measurementItems(prefix) {
		return {
			roomTemperature: {
				type: 'state',
	...
	_detailsSchema(prefix, id, groupId, thermostatId) {
		const measurements = this._measurementItems(id);
		return {
			type: 'tabs',
			items: {
				info: {
					...
					items: {
						...
						_d1m: { type: 'divider', color: 'primary' },
						_h1m: {
							type: 'header',
							text: { en: 'Measurements', de: 'Messungen' },
							sm: 12,
							newLine: true,
						},
						...measurements,
