...
class SchlueterThermostat extends utils.Adapter {
	...
	_formatIsoSssZ(value) {
		...
		// best-effort normalize common ISO-ish variants to "...sssZ"
	...
	async onStateChange(id, state) {
		...
		const comfortEndTime = this._formatIsoSssZ(
			this.thermostatComfortEnd[thermostatId] || this._nowPlusMinutesIso(120),
		);
		const boostEndTime = this._formatIsoSssZ(this.thermostatBoostEnd[thermostatId] || this._nowPlusMinutesIso(60));
		...
		const baseUpdate = {
			...
			// keep originals if present; otherwise send a valid sssZ value
			VacationBeginDay:
				this._formatIsoSssZ(this.thermostatVacationBegin?.[thermostatId]) || '1970-01-01T00:00:00.000Z',
			VacationEndDay:
				this._formatIsoSssZ(this.thermostatVacationEnd?.[thermostatId]) || '1970-01-01T00:00:00.000Z',
