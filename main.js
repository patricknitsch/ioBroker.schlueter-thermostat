...
class SchlueterThermostat extends utils.Adapter {
  ...
  async safeSetObjectNotExists(id, obj) {
    try {
      ...
      if (this.unloading || this._isConnClosed(e)) return;
      throw e;
    }
  }
  ...
  async pollOnce() {
    ...
    if (this.unloading || this.pollInFlight) return;
    ...
    this.log.debug(`Reading groups done: count=${groups.length}`);
      ...
      if (!this.unloading) this.log.warn(`Poll error: ${err?.message || err}`);
    }).finally(() => {
      this.pollInFlight = false;
    });
  ...
  async upsertGroup(g) {
    this.log.debug(`upsertGroup(): GroupId=${g.groupId}, SerialNumber=${g.serialNumber || ''}, ThermostatId=${g.thermostatId || ''}`);
    ...
    // cache ids
    if (g.serialNumber) this.groupSerial[groupId] = String(g.serialNumber);
    if (g.thermostatId) this.groupThermostatId[groupId] = String(g.thermostatId);
    if (g.thermostatName) this.groupNameCache[groupId] = String(g.thermostatName);
    if (g.comfortEndTime) this.groupComfortEnd[groupId] = String(g.comfortEndTime);
    ...
    await ensureState(`${devId}.online`, { name: 'Online', type: 'boolean', role: 'indicator.reachable', read: true, write: false });
    await ensureState(`${devId}.heating`, { name: 'Heating', type: 'boolean', role: 'indicator.working', read: true, write: false });

    await ensureState(`${devId}.temperature.room`, { name: 'Room temperature', type: 'number', role: 'value.temperature', unit: '°C', read: true, write: false });
    await ensureState(`${devId}.temperature.floor`, { name: 'Floor temperature', type: 'number', role: 'value.temperature', unit: '°C', read: true, write: false });

    await ensureState(`${devId}.setpoint.manual`, { name: 'Manual setpoint', type: 'number', role: 'value.temperature', unit: '°C', read: true, write: false });
    await ensureState(`${devId}.setpoint.comfort`, { name: 'Comfort setpoint', type: 'number', role: 'value.temperature', unit: '°C', read: true, write: false });

    await ensureState(`${devId}.regulationMode`, { name: 'Regulation mode', type: 'number', role: 'value', read: true, write: false });

    // writable
    await ensureState(`${devId}.setpoint.manualSet`, { name: 'Set manual setpoint', type: 'number', role: 'level.temperature', unit: '°C', read: true, write: true });
    await ensureState(`${devId}.setpoint.comfortSet`, { name: 'Set comfort setpoint', type: 'number', role: 'level.temperature', unit: '°C', read: true, write: true });
    await ensureState(`${devId}.regulationModeSet`, { name: 'Set regulation mode', type: 'number', role: 'level', read: true, write: true });

    // Schedule & Energy channels
    await this.safeSetObjectNotExists(`${devId}.schedule`, { type: 'channel', common: { name: 'Schedule' }, native: {} });
    await this.safeSetObjectNotExists(`${devId}.energy`, { type: 'channel', common: { name: 'Energy' }, native: {} });
    ...
    if (rt !== null) this.safeSetState(`${devId}.temperature.room`, { val: rt, ack: true });
    if (ft !== null) this.safeSetState(`${devId}.temperature.floor`, { val: ft, ack: true });
    ...
    this.safeSetState(`${devId}.regulationModeSet`, { val: mode, ack: true });
		...
		// Vacation
		const vEnabled = typeof g.vacationEnabled === 'boolean' ? Boolean(g.vacationEnabled) : Boolean(this.groupVacationEnabled[groupId]);
    ...
    if (g.schedule && Array.isArray(g.schedule.Days)) {
      for (const day of g.schedule.Days) {
        ...
        await this.safeSetObjectNotExists(dayCh, { type: 'channel', common: { name: `Day ${wd}` }, native: {} });

        const events = Array.isArray(day.Events) ? day.Events : [];
        for (let i = 0; i < events.length; i++) {
          const ev = events[i];
          const evCh = `${dayCh}.event${i}`;
          await this.safeSetObjectNotExists(evCh, { type: 'channel', common: { name: `Event ${i}` }, native: {} });

          await ensureState(`${evCh}.type`, { name: 'ScheduleType', type: 'number', role: 'value', read: true, write: false });
          await ensureState(`${evCh}.time`, { name: 'Clock', type: 'string', role: 'text', read: true, write: false });
          await ensureState(`${evCh}.temperature`, { name: 'Temperature', type: 'number', role: 'value.temperature', unit: '°C', read: true, write: false });
          await ensureState(`${evCh}.active`, { name: 'Active', type: 'boolean', role: 'indicator', read: true, write: false });
          await ensureState(`${evCh}.nextDay`, { name: 'EventIsOnNextDay', type: 'boolean', role: 'indicator', read: true, write: false });
          ...
          if (temp !== null) this.safeSetState(`${evCh}.temperature`, { val: temp, ack: true });
    ...
    if (client && tid) {
      try {
        ...
        await ensureState(`${devId}.energy.count`, { name: 'Values count', type: 'number', role: 'value', read: true, write: false });
        this.safeSetState(`${devId}.energy.count`, { val: usage.length, ack: true });

        for (let i = 0; i < usage.length; i++) {
          const sid = `${devId}.energy.value${i}`;
          await ensureState(sid, { name: `Energy value ${i}`, type: 'number', role: 'value.energy', unit: 'kWh', read: true, write: false });
          const v = Number(usage[i]?.EnergyKWattHour);
          if (Number.isFinite(v)) this.safeSetState(sid, { val: v, ack: true });
        }
      } catch (e) {
  ...
  async onStateChange(id, state) {
    ...
    try {
      if (sub === 'setpoint.manualSet') {
        ...
        await client.updateThermostat(serial, { ThermostatName: thermostatName, ComfortEndTime: comfortEndTime, RegulationMode: 4, ManualModeSetpoint: cToNum(tempC) });
        this.safeSetState(id, { val: tempC, ack: true });
      } else if (sub === 'setpoint.comfortSet') {
        const tempC = Number(state.val);
        await client.updateThermostat(serial, { ThermostatName: thermostatName, ComfortEndTime: comfortEndTime, ComfortSetpoint: cToNum(tempC) });
        this.groupComfortEnd[groupId] = comfortEndTime;
        this.safeSetState(id, { val: tempC, ack: true });
      } else if (sub === 'regulationModeSet') {
        const mode = Number(state.val);
        await client.updateThermostat(serial, { ThermostatName: thermostatName, ComfortEndTime: comfortEndTime, RegulationMode: mode });
